...
### Bài B3 — Debug HTML
Lỗi 1: Dòng 1 — Khai báo DOCTYPE sai cú pháp — Sửa thành <!DOCTYPE html>. Lỗi 2: Dòng 4 — Thẻ <title> không có thẻ đóng — Sửa thành <title>Trang web</title>. Lỗi 3: Dòng 5 — Thuộc tính charset viết sai — Sửa thành <meta charset="UTF-8">. Lỗi 4: Dòng 8 — Thẻ <h1> đóng sai bằng thẻ mở — Sửa <h1> ở cuối thành </h1>. Lỗi 5: Dòng 12 — Thẻ <a> thứ nhất đóng sai bằng thẻ mở — Sửa <a> ở cuối thành </a>. Lỗi 6: Dòng 20 — Thẻ <img> thiếu ngoặc kép ở src và thiếu thuộc tính alt bắt buộc — Sửa thành <img src="iphone.jpg" alt="iPhone 16 Pro">. Lỗi 7: Dòng 22 — Lỗi chồng chéo thẻ (nesting error) giữa <p> và <b> — Sửa thành <p>Giá: <b>25.990.000đ</b></p>. Lỗi 8: Dòng 27 — Bảng <table> thiếu các thẻ phân chia cấu trúc ngữ nghĩa (Semantic) — Cần bổ sung <thead> (chứa hàng Tên, Giá) và <tbody> (chứa hàng iPhone, 25tr). Lỗi 9: Dòng 38 — Lỗi Semantic: Thẻ <main> được sử dụng 2 lần trong một trang (Mỗi trang chỉ được có 1 thẻ main) — Đổi thẻ <main> thứ hai thành <aside> hoặc gộp vào main phía trên. Lỗi 10: Dòng 43 — Thẻ <p> trong footer thiếu thẻ đóng và thiếu thẻ đóng của HTML ở cuối cùng — Thêm </p> ở dòng 43 và </html> ở dòng cuối cùng.
### Bài B4 — Phân tích trang web thật (Trang web chọn: shopee.vn)
1. Phân tích thẻ Semantic: Screenshot Semantic

3 thẻ Semantic đúng:
<header>: Nằm ở trên cùng, chứa logo Shopee và thanh tìm kiếm.
<nav>: Dùng cho danh sách các link điều hướng phụ phía trên thanh tìm kiếm.
<footer>: Nằm ở dưới cùng, chứa thông tin công ty, chính sách, app tải xuống.
Thẻ KHÔNG dùng đúng semantic (Lỗi Div Soup):
<div class="shopee-searchbar">: Shopee dùng thẻ div thay vì thẻ <form> cho khu vực tìm kiếm.
<div class="product-card">: Dùng div thay vì <article> cho từng ô sản phẩm riêng biệt.
2. Phân tích Table: Screenshot Table

Table này hiển thị nội dung: Bảng thông số kỹ thuật chi tiết của điện thoại.
Có dùng <thead>, <tbody> không?: Có sử dụng <tbody>, nhưng đôi khi lược bỏ <thead> nếu bảng chỉ có 2 cột key-value.
3. Phân tích Form: Screenshot Form

Form được dùng ở: Khu vực Đăng nhập / Tìm kiếm sản phẩm.
Thuộc tính: action="/search" và method="get" (để đưa query lên URL).
Input types được dùng: <input type="text"> (nhập từ khóa), <input type="password"> (nếu ở trang đăng nhập).
### PHẦN C : SUY LUẬN
### CÂU C1
Nguồn tham chiếu:
